...
public class DengueForecastService : IPredictionService<DengueForecastInput, DengueForecastOutput>
{
    ...
    private void LoadModelIfExists()
    {
        if (File.Exists(_modelPath))
        {
            ...
            {
                _model = _mlContext.Model.Load(_modelPath, out _);
                _logger.LogInformation("Model loaded successfully from {Path}", _modelPath);
            ...
            {
                _logger.LogError(ex, "Failed to load model from {Path}", _modelPath);
            }
        }
        else
        {
    ...
    private void LoadMetricsIfExists()
    {
        if (File.Exists(_metricsPath))
        {
            ...
            {
                _logger.LogWarning(ex, "Failed to load metrics from {Path}", _metricsPath);
    ...
    private async Task SaveMetricsAsync()
    {
        ...
        await File.WriteAllTextAsync(_metricsPath, json);
        _logger.LogInformation("Metrics saved to {Path}", _metricsPath);
    }

    private double CalculateStandardDeviation(IDataView predictions)
    {
    ...
    private void EnrichPredictionWithStatistics(DengueForecastOutput prediction, double confidenceLevel)
    {
    ...
    private double CalculateOutbreakProbability(float predictedCases, float lowerBound, float upperBound)
    {
...
{
    public float DengueCount { get; set; }
    public float Score { get; set; }
}

public class SavedMetrics
{

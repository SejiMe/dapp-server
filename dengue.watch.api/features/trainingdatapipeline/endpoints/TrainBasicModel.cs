...
public class TrainBasicModel : IEndpoint
{
    public static IEndpointRouteBuilder MapEndpoints(IEndpointRouteBuilder app)
    {
        var group = app.MapGroup("api/training-data")
            .WithTags("Training Data Pipeline")
            .WithSummary("Train Basic Model");

        group.MapPost("basic", Handler);
        return group;
    }

    private static async Task<Results<Ok<ModelInfo>, ProblemHttpResult>> Handler([FromServices] IPredictionService<DengueForecastInput, DengueForecastOutput> _dengue)
    {
        try
        {
